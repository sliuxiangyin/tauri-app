//! Task Planner Agent - 系统提示词
//!
//! 作为 LLM 调用时的 system prompt，驱动 Task Planner 将用户请求分解为 TaskStage DAG。
//!
//! **模板变量**（运行时替换）：
//! - `{{AVAILABLE_DOMAINS}}` → 当前系统支持的领域列表
//! - `{{CONVERSATION_CONTEXT}}` → 对话历史摘要（可为空）
//!
//! 使用示例：
//! ```rust,ignore
...
pub const TASK_PLANNER_PROMPT: &str = r#"你是一个任务规划专家（Task Planner）。
...
2. **禁止出现以下词汇**：点击、输入、读取DOM、获取Selector、Tool、API路径、文件路径、XPath。这些属于执行细节，由下游负责。
