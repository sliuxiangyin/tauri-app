...
//! - `{{AVAILABLE_TOOLS}}` → 当前领域可用工具列表（名称 + 能力描述）
//! - `{{RUNTIME_CONTEXT}}` → 运行环境信息
//! - `{{PREVIOUS_STAGE_OUTPUTS}}` → 前序 Stage 的实际输出值
//! - `{{PREVIOUS_ATTEMPT_ERRORS}}` → 上次 Layer 2 replan 时失败的步骤信息（首次规划为空）
//!
//! 使用示例：
//! ```rust,ignore
...
### 上次尝试失败的步骤（仅 Layer 2 replan 时非空）
...
9. **基于失败改进（仅 re-plan 时）**。若 `{{PREVIOUS_ATTEMPT_ERRORS}}` 不为空，说明这是 Layer 2 re-plan：
   - **避免重复同样的失败模式**（如上次 selector 错误 → 本次应改用更稳健的定位策略）
   - **必要时增加验证步骤**（如先 snapshot 再 selector，确保元素存在后再操作）
   - **可调整 Step 顺序**，把关键的前置条件提前
   - **可修改 `expected_tool`**，选择不同的工具完成相同语义
   - 不要简单复制上次的 plan——LLM 必须根据失败信息**显式改进**
...
- **类型**：工具来源类型（如 `mcp` = MCP 工具，后续可能扩展 `builtin`、`plugin` 等）
- **服务**：工具所属服务/Server 名称（如 `browser`、`fs`、`adb`）
- **操作**：具体操作方法（如 `navigate`、`click`、`fill`、`read`）

示例：
- `mcp__browser__navigate` → 导航到指定 URL
- `mcp__browser__click` → 点击页面元素
- `mcp__fs__read` → 读取文件内容
...
ExecutionPlan:
- step 1: { goal: "分析搜索结果数据，提取关键信息并生成结构化摘要", depends_on: [], expected_tool: "llm_reasoning" }
