...
export const PromptInputTextarea = ({
  ...
  const handleKeyDownMerged: KeyboardEventHandler<HTMLTextAreaElement> = (
    e
  ) => {
    handleKeyDown(e);
    onKeyDownProp?.(e);
  ...
  return (
    <InputGroupTextarea
      className={cn("[field-sizing:fixed] max-h-48 min-h-16", className)}
      name="message"
      onCompositionEnd={handleCompositionEnd}
      onCompositionStart={handleCompositionStart}
      onKeyDown={handleKeyDownMerged}
      onPaste={handlePaste}
...
/** Web Speech API 实例形状（避免与 lib.dom 中 `SpeechRecognition` 全局声明冲突）。 */
type LegacySpeechRecognition = {
  ...
  start(): void;
  stop(): void;
  onstart: ((this: LegacySpeechRecognition, ev: Event) => unknown) | null;
  onend: ((this: LegacySpeechRecognition, ev: Event) => unknown) | null;
  onresult:
    | ((
        this: LegacySpeechRecognition,
        ev: LegacySpeechRecognitionResultEvent
      ) => unknown)
    | null;
  onerror:
    | ((this: LegacySpeechRecognition, ev: { error: string }) => unknown)
    | null;
};

type LegacySpeechRecognitionResultEvent = {
  resultIndex: number;
  results: {
...
function getLegacySpeechRecognitionCtor():
  | (new () => LegacySpeechRecognition)
  | null {
...
export const PromptInputSpeechButton = ({
  ...
  const [recognition, setRecognition] = useState<LegacySpeechRecognition | null>(
    null
  );
  const recognitionRef = useRef<LegacySpeechRecognition | null>(null);

  useEffect(() => {
    const Ctor = getLegacySpeechRecognitionCtor();
    if (Ctor) {
